...
import re
r= re.compile('{\'src\':.*\'message\':.*}')
def checkMessageFormat(message):
    if r.match(message):
	return 1
    return 0

#should not be a global. for testing echohandler
...
#separate messages that have possibly collided in the tcp stream buffer
    msg = []
    i = message.find('src')
    while(i > 0):
	tmp = i
	i = message.find('src', i +1)
	if (i == -1):
		if checkMessageFormat(message[tmp-2:]):			
			msg.append(message[tmp-2:])
	else:
		if checkMessageFormat(message[tmp-2:i-2]):					
			msg.append(message[tmp-2:i-2])
